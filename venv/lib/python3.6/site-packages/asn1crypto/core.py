...
CLASS_NUM_TO_NAME_MAP = {0: "universal", 1: "application", 2: "context", 3: "private"}
...
METHOD_NUM_TO_NAME_MAP = {0: "primitive", 1: "constructed"}
    ...
    def dump(self, force=False):
        ...
        if self._header is None or force:
            ...
            if self.explicit is not None:
                for class_, tag in self.explicit:
                    self._header = (
                        _dump_header(class_, 1, tag, self._header + self.contents)
                        + self._header
                    )
        ...
        if self._native is None:
            ...
            if not self._indefinite and extra_bits == 0:
                ...
                if self._indefinite and extra_bits > 0:
                    raise ValueError(
                        "Constructed bit string has extra bits on indefinite container"
                    )
    ...
    def _parse_children(self, recurse=False):
        ...
        try:
            ...
            while again:
                ...
                if field < field_len:
                    _, field_spec, value_spec, field_params, spec_override = cls._precomputed_specs[
                        field
                    ] or self._determine_spec(
                        field
                    )
    ...
    def _parse_children(self, recurse=False):
        ...
        try:
            ...
            for index in range(0, total_fields):
                ...
                name, field_spec, value_spec, field_params, spec_override = cls._precomputed_specs[
                    index
                ] or self._determine_spec(
                    index
                )
    ...
    if spec is not None:
        ...
        if not no_explicit and (
            ...
            for expected_class, expected_tag in explicit_info:
                ...
                parsed_class, parsed_method, parsed_tag, parsed_header, to_parse, parsed_trailer = (
                    info
                )
