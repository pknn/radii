...
class SchemaObjects(object):
    ...
    def check_constraint(self, name, source, condition, schema=None, **kw):
        t = sa_schema.Table(
            source, self.metadata(), sa_schema.Column("x", Integer), schema=schema
        )
    ...
    def generic_constraint(self, name, table_name, type_, schema=None, **kw):
        ...
        try:
            ...
            raise TypeError(
                "'type' can be one of %s" % ", ".join(sorted(repr(x) for x in types))
            )
        else:
    ...
    def index(self, name, tablename, columns, schema=None, **kw):
        ...
        idx = sa_schema.Index(
            name, *[util.sqla_compat._textual_index_column(t, n) for n in columns], **kw
        )
        return idx
