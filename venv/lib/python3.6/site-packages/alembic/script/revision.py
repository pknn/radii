...
class RevisionMap(object):
        ...
        for rev in map_.values():
            for downrev in rev._all_down_revisions:
                if downrev not in map_:
                    util.warn(
                        "Revision %s referenced from %s is not present" % (downrev, rev)
                    )
            ...
            for branch_label in revision._orig_branch_labels:
                if branch_label in map_:
                    raise RevisionError(
                        ...
                        "used by revision %s"
                        % (branch_label, revision.revision, map_[branch_label].revision)
                    )
    ...
    def add_revision(self, revision, _replace=False):
        ...
        if revision._is_real_head:
            self._real_heads = tuple(
                ...
                if head
                not in set(revision._all_down_revisions).union([revision.revision])
            ) + (revision.revision,)
        if revision.is_head:
            self.heads = tuple(
                head
    ...
    def get_current_head(self, branch_label=None):
        ...
        if len(current_heads) > 1:
            raise MultipleHeads(
                current_heads, "%s@head" % branch_label if branch_label else "head"
            )
    ...
    def get_revisions(self, id_):
        ...
        if isinstance(id_, (list, tuple, set, frozenset)):
            ...
            return tuple(
                self._revision_for_ident(rev_id, branch_label) for rev_id in resolved_id
            )
    ...
    def _revision_for_ident(self, resolved_id, check_branch=None):
        ...
        if revision is False:
            ...
            if not revs:
                raise ResolutionError(
                    "No such revision or branch '%s'" % resolved_id, resolved_id
                )
            elif len(revs) > 1:
                raise ResolutionError(
    ...
    def iterate_revisions(
        ...
        relative_upper = self._relative_iterate(
            upper, lower, True, implicit_base, inclusive, assert_relative_length
        )
        if relative_upper:
            return relative_upper

        relative_lower = self._relative_iterate(
            lower, upper, False, implicit_base, inclusive, assert_relative_length
        )
        if relative_lower:
            ...
            if check:
                ...
                if overlaps:
                    raise RevisionError(
                        ...
                        % (target.revision, ", ".join(r.revision for r in overlaps))
                    )
    ...
    def _iterate_revisions(
        ...
        if limit_to_lower_branch:
            ...
            base_lowers = set()
            candidate_lowers = upper_ancestors.difference(lower_ancestors).difference(
                lower_descendants
            )
        ...
        if not total_space:
            ...
            start_from.update(
                self._get_ancestor_nodes(list(start_from), include_dependencies=True)
            )
...
class Revision(object):
    ...
    def _all_down_revisions(self):
        return (
            util.to_tuple(self.down_revision, default=()) + self._resolved_dependencies
        )
