...
class ScriptDirectory(object):
    ...
    def _catch_revision_errors(
        self, ancestor=None, multiple_heads=None, start=None, end=None, resolution=None
    ):
        try:
            yield
    ...
    def get_base(self):
        ...
        if len(bases) > 1:
            raise util.CommandError(
                "The script directory has multiple bases. " "Please use get_bases()."
            )
        elif bases:
            return bases[0]
        else:
    ...
    def _downgrade_revs(self, destination, current_rev):
        with self._catch_revision_errors(
            ...
            return [
                migration.MigrationStep.downgrade_from_script(self.revision_map, script)
                for script in revs
            ]
            ...
            for dest in dests:
                if dest is None:
                    ...
                    steps.extend(
                        [
                            migration.StampStep(
                                head.revision, None, False, True, self.revision_map
                            )
                ...
                if descendants.intersection(filtered_heads):
                    ...
                    step = migration.StampStep(
                        todo_heads, dest.revision, False, False, self.revision_map
                    )
                    steps.append(step)
                    continue
                    ...
                    todo_heads = [head.revision for head in filtered_heads]
                    step = migration.StampStep(
                        todo_heads, dest.revision, True, False, self.revision_map
                    )
                    steps.append(step)
                    continue
                else:
