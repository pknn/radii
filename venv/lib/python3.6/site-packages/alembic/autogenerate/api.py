...
def produce_migrations(context, metadata):
    ...
    migration_script = ops.MigrationScript(
        rev_id=None, upgrade_ops=ops.UpgradeOps([]), downgrade_ops=ops.DowngradeOps([])
    )
...
def _render_migration_diffs(context, template_args):
    ...
    migration_script = ops.MigrationScript(
        rev_id=None, upgrade_ops=upgrade_ops, downgrade_ops=upgrade_ops.reverse()
    )
...
class RevisionContext(object):
    ...
    def __init__(
        self, config, script_directory, command_args, process_revision_directives=None
    ):
        self.config = config
