...
def _autogen_for_tables(autogen_context, upgrade_ops, schemas):
    ...
    _compare_tables(
        conn_table_names, metadata_table_names, inspector, upgrade_ops, autogen_context
    )
...
def _compare_tables(
    ...
    for s, tname in metadata_table_names.difference(conn_table_names):
        ...
        if autogen_context.run_filters(metadata_table, tname, "table", False, None):
            upgrade_ops.ops.append(ops.CreateTableOp.from_table(metadata_table))
            log.info("Detected added table %r", name)
            modify_table_ops = ops.ModifyTableOps(tname, [], schema=s)

            comparators.dispatch("table")(
                autogen_context, modify_table_ops, s, tname, None, metadata_table
            )
...
def _make_unique_constraint(params, conn_table):
    uq = sa_schema.UniqueConstraint(
        *[conn_table.c[cname] for cname in params["column_names"]], name=params["name"]
    )
...
def _compare_indexes_and_uniques(
    ...
    def obj_added(obj):
        if obj.is_index:
            if autogen_context.run_filters(obj.const, obj.name, "index", False, None):
    ...
    def obj_removed(obj):
        if obj.is_index:
            ...
            if autogen_context.run_filters(obj.const, obj.name, "index", True, None):
            ...
            if autogen_context.run_filters(
                ...
                log.info(
                    "Detected removed unique constraint '%s' on '%s'", obj.name, tname
                )
...
def _correct_for_uq_duplicates_uix(
    ...
    metadata_uq_names = set(
        [cons.name for cons in metadata_unique_constraints if cons.name is not None]
    )
    ...
    metadata_ix_names = set([cons.name for cons in metadata_indexes if cons.unique])
...
def _compare_nullable(
    autogen_context, alter_column_op, schema, tname, cname, conn_col, metadata_col
):
...
def _compare_server_default(
    ...
    isdiff = autogen_context.migration_context._compare_server_default(
        conn_col, metadata_col, rendered_metadata_default, rendered_conn_default
    )
    if isdiff:
