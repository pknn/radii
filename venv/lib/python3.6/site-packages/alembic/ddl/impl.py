...
class DefaultImpl(with_metaclass(ImplMeta)):
    ...
    def _exec(
        ...
        if self.as_sql:
            ...
            self.static_output(
                text_type(construct.compile(dialect=self.dialect, **compile_kw))
                .replace("\t", "    ")
                .strip()
    ...
    def compare_type(self, inspector_column, metadata_column):
        ...
        if hasattr(metadata_impl, "compare_against_backend"):
            comparison = metadata_impl.compare_against_backend(self.dialect, conn_type)
    ...
    def correct_for_autogen_constraints(
        self, conn_uniques, conn_indexes, metadata_unique_constraints, metadata_indexes
    ):
        pass
...
def _numeric_compare(t1, t2):
    return (t1.precision is not None and t1.precision != t2.precision) or (
        t1.precision is not None and t1.scale is not None and t1.scale != t2.scale
    )
