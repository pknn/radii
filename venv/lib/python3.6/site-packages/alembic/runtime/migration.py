...
class MigrationContext(object):
    ...
    def __init__(self, dialect, connection, opts, environment_context=None):
        ...
        if "output_encoding" in opts:
            self.output_buffer = EncodedIO(
                opts.get("output_buffer") or sys.stdout, opts["output_encoding"]
            )
        else:
        ...
        if opts.get("version_table_pk", True):
            self._version.append_constraint(
                PrimaryKeyConstraint("version_num", name="%s_pkc" % version_table)
            )
    ...
    def get_current_heads(self):
        ...
        if self.as_sql:
            ...
            if start_from_rev == "base":
                ...
                start_from_rev = self.script.get_revision(start_from_rev).revision
    ...
    def run_migrations(self, **kw):
        ...
        starting_in_transaction = not self.as_sql and self._in_connection_transaction()
...
class HeadMaintainer(object):
    ...
    def update_to_step(self, step):
        if step.should_delete_branch(self.heads):
            ...
            (delete_revs, update_from_rev, update_to_rev) = step.merge_branch_idents(
                self.heads
            )
            log.debug(
            ...
            (update_from_rev, update_to_rev, insert_revs) = step.unmerge_branch_idents(
                self.heads
            )
            log.debug(
